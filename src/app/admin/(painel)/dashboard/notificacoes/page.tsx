    ...
    <div className="flex w-full flex-1 flex-col gap-6 px-4 py-6 sm:px-6 lg:px-10 lg:py-8">
      ...
      <div className="flex items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          ...
          <Link href="/admin/dashboard" className="flex items-center gap-1 text-sm text-gray-500 hover:text-gray-900 dark:text-gray-400 dark:hover:text-gray-50">
            <ChevronLeft className="h-4 w-4" aria-hidden="true" /> Dashboard
          </Link>
        </div>
        {/* Sem sino: seria um atalho para esta mesma página. */}
        <HeaderActions showBell={false} />
      </div>
      ...
      >
        ...
        {(pagina > 1 || feed.hasMore) && (
          <div className="flex items-center justify-between border-t border-gray-100 pt-3 text-sm font-semibold dark:border-gray-800">
            {pagina > 1 ? (
              <Link href={`/admin/dashboard/notificacoes?pagina=${pagina - 1}`} className="flex items-center gap-1 text-gray-500 hover:text-gray-900 dark:text-gray-400 dark:hover:text-gray-50">
            ...
            {feed.hasMore && (
              <Link href={`/admin/dashboard/notificacoes?pagina=${pagina + 1}`} className="flex items-center gap-1 text-primary dark:text-blue-300">
